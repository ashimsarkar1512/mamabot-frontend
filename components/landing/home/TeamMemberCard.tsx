...
import Image from "next/image";
import { Twitter, Facebook, Linkedin } from "lucide-react";
import { TeamMember } from "@/redux/features/api/user/OurTeam/OurTeam";
  ...
  return (
    <div className="bg-[#F9FAFB] rounded-2xl shadow-md px-6 py-8 text-center hover:shadow-lg transition">
      ...
      <div className="flex justify-center mb-5">
        <div className="w-24 h-24 rounded-full overflow-hidden shadow">
          <Image
            src={member.thumbnail_img || "/images/default-avatar.png"}
            alt={member.name}
            width={96}
            height={96}
            className="object-cover"
      ...
      {/* Role */}
      <p className="text-sm text-[#677381] mt-1">{member.title}</p>

      {/* Social icons */}
      <div className="flex justify-center gap-4 mt-5 text-gray-400">
        {member.twitter_link && (
          <a href={member.twitter_link} target="_blank" rel="noreferrer" className="hover:text-primary">
            <Twitter size={16} />
          </a>
        )}
        {member.fb_link && (
          <a href={member.fb_link} target="_blank" rel="noreferrer" className="hover:text-primary">
            <Facebook size={16} />
          </a>
        )}
        {member.linkedin_link && (
          <a href={member.linkedin_link} target="_blank" rel="noreferrer" className="hover:text-primary">
            <Linkedin size={16} />
          </a>
        )}
      </div>
